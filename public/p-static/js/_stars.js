...
mouse.x = video.clientWidth / 2;
mouse.y = video.clientHeight / 2;
...
if ('ontouchstart' in document.documentElement && window.DeviceOrientationEvent) {
  ...
  window.addEventListener('deviceorientation', function(e) {
    mouse.x = (video.clientWidth / 2) - ((e.gamma / 90) * (video.clientWidth / 2) * 2);
    mouse.y = (video.clientHeight / 2) - ((e.beta / 90) * (video.clientHeight / 2) * 2);
    //console.log('Center: x:'+(video.clientWidth/2)+' y:'+(video.clientHeight/2));
...
/*
if (orbitTilt) {
  var tiltX = -(((video.clientWidth / 2) - mouse.x + ((nPos.x - 0.5) * noiseStrength)) * tilt),
    tiltY = (((video.clientHeight / 2) - mouse.y + ((nPos.y - 0.5) * noiseStrength)) * tilt);
...
function resize() {
video.width = window.innerWidth * (window.devicePixelRatio || 1);
video.height = video.width * (video.clientHeight / video.clientWidth);
}
...
switch (this.stage) {
  ...
  case 1:
    if (this.distances.length > 0) {
      ...
      var linkSpeedRel = linkSpeed * 0.00001 * video.width;
...
return {
  x: (x * video.width) + ((((video.width / 2) - mouse.x + ((nPos.x - 0.5) * noiseStrength)) * z) * motion),
  y: (y * video.height) + ((((video.height / 2) - mouse.y + ((nPos.y - 0.5) * noiseStrength)) * z) * motion)
};
}
