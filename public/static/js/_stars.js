...
*/
  ...
  if (
    ...
    window.addEventListener(
      ...
      function (e) {
        mouse.x =
          video.clientWidth / 2 - (e.gamma / 90) * (video.clientWidth / 2) * 2;
        mouse.y =
          video.clientHeight / 2 - (e.beta / 90) * (video.clientHeight / 2) * 2;
...
function position(x, y, z) {
  return {
    x:
      x * video.width +
      (video.width / 2 - mouse.x + (nPos.x - 0.5) * noiseStrength) * z * motion,
    y:
      y * video.height +
      (video.height / 2 - mouse.y + (nPos.y - 0.5) * noiseStrength) *
        z *
        motion,
  };
}
